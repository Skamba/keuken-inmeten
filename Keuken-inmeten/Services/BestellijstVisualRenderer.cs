...
public static class BestellijstVisualRenderer
{
    public static string Render(BestellijstVisualDocument document)
    {
        ...
        const double topBand = 24;
        const double bottomBand = 20;
        ...
        var svgHeight = height + topBand + bottomBand + padding;
        var paneelX = padding;
        var paneelY = topBand;
        ...
        sb.Append($"<text x=\"{Fmt(paneelX)}\" y=\"{Fmt(14)}\" font-size=\"8\" fill=\"#334155\" text-anchor=\"start\">Bovenzijde · nulpunt linksboven</text>");
        sb.Append($"<rect x=\"{Fmt(svgWidth - 42)}\" y=\"{Fmt(6)}\" width=\"36\" height=\"14\" rx=\"7\" fill=\"#0f4c81\" opacity=\"0.9\" />");
        sb.Append($"<text x=\"{Fmt(svgWidth - 24)}\" y=\"{Fmt(15.5)}\" font-size=\"7\" fill=\"#fff\" text-anchor=\"middle\" font-weight=\"700\">{Encode(document.RegelCode)}</text>");
        ...
        sb.Append($"<text x=\"{Fmt(paneelX + 7)}\" y=\"{Fmt(paneelY + 3)}\" font-size=\"7\" fill=\"#0f4c81\" text-anchor=\"start\">0,0</text>");

        if (document.Boorgaten.Count > 0)
        {
            var scharnierBadgeX = document.ScharnierZijde == ScharnierZijde.Links ? paneelX : paneelX + width - 58;
        ...
        sb.Append($"<text x=\"{Fmt(svgWidth / 2)}\" y=\"{Fmt(svgHeight - 4)}\" font-size=\"8\" fill=\"#334155\" text-anchor=\"middle\">{Encode(BestellijstExportFormatter.FormatZaagmaat(document.BreedteMm, document.HoogteMm))} · X vanaf links · Y vanaf boven</text>");
        sb.Append("</svg>");
